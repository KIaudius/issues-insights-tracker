...
test.describe("Issue Management", () => {
  ...
  test("can create a new issue", async ({ page }) => {
    ...
    await page
      ...
      .fill(
        "This is a test issue created during automated end-to-end testing.",
      );
    ...
    await page.goto("/issues");
    await expect(
      page.getByText("Test Issue Created by E2E Test"),
    ).toBeVisible();
  });
  ...
  test("can add comments to an issue", async ({ page }) => {
    ...
    await page
      ...
      .fill("This is a test comment from E2E testing");

    // Submit comment
    await page
      .getByRole("button", { name: /post|add|submit comment/i })
      .click();

    // Verify comment appears
    await expect(
      page.getByText("This is a test comment from E2E testing"),
    ).toBeVisible();
  });
